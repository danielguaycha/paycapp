...
class _AddCreditPageState extends State<AddCreditPage> {
    ...
    return Scaffold(
      ...
      bottomNavigationBar: new BottomAppBar(
        ...
        child: new Row(
          ...
          children: <Widget>[
            IconButton(onPressed: () =>  _clearCredit(context) , icon: Icon(Icons.clear_all), tooltip: 'Limpiar Formulario',),
          ],
        ),
      ),
      floatingActionButton: new FloatingActionButton(
        ...
        onPressed: !_enabled ? () {
          Alert.toast(context, "Seleccione un cliente", position: ToastPosition.center);
        } : () {
          saveCredit(context);
        },
      ),
  ...
  saveCredit(context) async {
    ...
    int isOk = await Alert.confirm(context, title: "Confirmar" ,content: "¿Está seguro que desea guardar este crédito?");
    if(isOk == 1){
      return;
    }
      ...
      child: Padding(
        ...
        child: Column(
          ...
          children: <Widget>[ 
            Row(
              children: <Widget>[
                Expanded(child: Text("PRENDA", style: TextStyle(color: Colors.black54,fontSize: 10))),                
                Checkbox(                  
                  onChanged: (val){
                    setState(() {
                      _prenda = val;
  ...
  _clientField(context) {
    return TextFormField(
      ...
      decoration: (_enabled) ? InputDecoration(
        labelText: 'Cliente',
  ...
  _zoneField(context){
    return TextFormField(
      ...
      readOnly: true,
      textCapitalization: TextCapitalization.words,
      decoration: (_route != null) ? InputDecoration(
  ...
  _setGeoLocField(){
    return CheckboxListTile(      
      ...
      subtitle: Text('¿Esta ubicación es la dirección del cliente?', style: TextStyle(fontSize: 11),)          
    );
  }  
  ...
  _photosBtn({ String select: 'Seleccionar', String take: 'Capturar', 
          String remove: 'Quitar', bool prenda: false }){
    
    final img = prenda ? _credit.prendaImg : _credit.refImg;

    return  Row(
      mainAxisAlignment: MainAxisAlignment.center,
      children:(img ==null) ? <Widget>[
        FlatButton.icon(
  ...
  searchZone(context) async{
      _route = await Navigator.push(context,MaterialPageRoute(builder: (context) => RouteSelectionPage()));
    ...
    return Row(
      ...
      children: <Widget>[
        Column(
          ...
          children: <Widget>[
            Container(
              child: Text('Utilidad \$: ${_credit.totalUtilidad.toStringAsFixed(2)}'),
            ),
            Container(
  ...
  _openGallery({ bool prenda: false }) async {
      ...
      if(!prenda) _credit.refImg = img;
      else _credit.prendaImg = img;
